...
class AwareConfigForm(forms.Form):
    ...
    extra = util.JsonConfigFormField(help_text="Extra data?", required=False, yaml=True, widget=forms.Textarea(attrs={'style': 'font-family: monospace; width: 1000px; height: 500px'}))
    ...
    if ('schedule' in sched
       ...
       and'random_intervals' in sched['schedule']['trigger']):
        ...
        for day_n in range(forward_time//(3600*24) + 1):
            ...
            times = util.random_intervals(start=start_ts, end=end_ts, N=params['N'], min_spacing=params.get('min_spacing', 0),
                                          max_spacing=params['max_spacing'] if 'max_spacing' in params else None,
