...
_iterations = 135
_salt = b'\xa6\xab\x09\x93\xf4\xcc\xee\x10'
_key_size = 8
_block_size = 8
...
#    return ''.join([chr(set_parity(ord(digit))) for digit in key]) 
    return bytes(bytearray([set_parity(digit) for digit in key]))
...
def remove_padding(data):
    ...
    if data_size == 0:
        return data
    num_padding_bytes = struct.unpack('B', data[data_size-1:data_size])[0]
...
import io
def decrypt2(data, key, password=None):
    ...
    #output = [ ]
    #encrypted_file = io.BytesIO(data)
    #while True:
    #    chunk = encrypted_file.read(_block_size)
    #    #print(chunk)
    #    data = decryptor.decrypt(chunk)
    #    #print('  ', data)
    #    if not chunk:
    #        break
    #    output.append(data)
    #output[-1] = remove_padding(output[-1])
